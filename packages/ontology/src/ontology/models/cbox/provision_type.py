...
def _object_meta() -> ObjectMeta:
    return ObjectMeta(
        type_="ProvisionType",
        description=(
            "A classification of contract provisions by clause family "
            "(events of default, negative pledge, pari passu, collective "
            "action clause)."
        ),
...
class ProvisionType(BaseResource):
    """A classification of contract provisions by clause family (events of
    default, negative pledge, pari passu, collective action clause)."""
    ...
    id: Annotated[Iri, named_individual_iri_enum("ProvisionType")] = Field(alias="@id")
    type_: Literal["ProvisionType"] = Field(alias="@type")
