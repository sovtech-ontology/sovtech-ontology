...
def _object_meta() -> ObjectMeta:
    return ObjectMeta(
        type_="InterestRateType",
        description=(
            "A classification of interest rate structures (fixed, floating, "
            "zero-coupon, step-up)."
        ),
...
class InterestRateType(BaseResource):
    """A classification of interest rate structures (fixed, floating,
    zero-coupon, step-up)."""
    ...
    id: Annotated[AnyUrl, named_individual_iri_enum("InterestRateType")] = Field(
        alias="@id"
    )
    type_: Literal["InterestRateType"] = Field(alias="@type")
