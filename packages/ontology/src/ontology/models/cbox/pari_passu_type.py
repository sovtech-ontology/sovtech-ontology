...
def _object_meta() -> ObjectMeta:
    return ObjectMeta(
        type_="PariPassuType",
        description=(
            "A classification of pari passu clause wording: old "
            "(ratable-payment-exposed) versus enhanced (ranking-only)."
        ),
        named_individuals=named_individuals,
    )


class PariPassuType(BaseResource):
    """A classification of pari passu clause wording: old
    (ratable-payment-exposed) versus enhanced (ranking-only)."""
    ...
    id: Annotated[Iri, named_individual_iri_enum("PariPassuType")] = Field(alias="@id")
    type_: Literal["PariPassuType"] = Field(alias="@type")
