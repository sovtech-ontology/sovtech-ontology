...
def _object_meta() -> ObjectMeta:
    return ObjectMeta(
        type_="DocumentSectionType",
        description=(
            "A classification of document text parts by the disclosure "
            "section they belong to (risk factors, definitions, use of "
            "proceeds)."
        ),
...
class DocumentSectionType(BaseResource):
    """A classification of document text parts by the disclosure section they
    belong to (risk factors, definitions, use of proceeds)."""
    ...
    id: Annotated[AnyUrl, named_individual_iri_enum("DocumentSectionType")] = Field(
        alias="@id"
    )
    type_: Literal["DocumentSectionType"] = Field(alias="@type")
