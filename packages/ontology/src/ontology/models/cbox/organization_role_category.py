...
def _object_meta() -> ObjectMeta:
    return ObjectMeta(
        type_="OrganizationRoleCategory",
        description=(
            "A broad classification of the kind of role an organization "
            "plays on a debt instrument or agreement."
        ),
        named_individuals=named_individuals,
    )


class OrganizationRoleCategory(BaseResource):
    """A broad classification of the kind of role an organization plays on a
    debt instrument or agreement."""
    ...
    id: Annotated[Iri, named_individual_iri_enum("OrganizationRoleCategory")] = Field(
        alias="@id"
    )
    type_: Literal["OrganizationRoleCategory"] = Field(alias="@type")
