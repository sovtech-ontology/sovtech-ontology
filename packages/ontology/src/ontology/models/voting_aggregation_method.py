...
def _object_meta() -> ObjectMeta:
    return ObjectMeta(
        type_="VotingAggregationMethod",
        description=(
            "How bondholder modification votes aggregate across series: "
            "single series, cross-series single aggregated, or cross-series "
            "two-tier."
        ),
...
class VotingAggregationMethod(BaseResource):
    """How bondholder modification votes aggregate across series: single
    series, cross-series single aggregated, or cross-series two-tier."""
    ...
    id: Annotated[AnyUrl, named_individual_iri_enum("VotingAggregationMethod")] = Field(
        alias="@id"
    )
    type_: Literal["VotingAggregationMethod"] = Field(alias="@type")
